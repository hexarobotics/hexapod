...
// El voltaje m�nimo en una celda debe ser de 3.0V
//El voltaje m�ximo en una celda de 4.2V
int m=8740;
//int n=17777;
...
#define zumbador  11
float nivel=(PI/6)/128; //2^7 niveles de comapraci�n		 	//60�--> pi/3		//  Variable que indica el minimo valor en radianes que se puede girar
...
void setup()
{
	...
	Setup_Servo();
	/*
	//MANDAMOS LA POSICI�N DE REPOSO AL ROBOT
	...
	//Se calcula la cinem�tica inversa para poner a los servos en la posici�n inicial para el movimiento
		doIK();
		Interpolate_Setup(1500);//  esta primera interpolaci�n la hacemos lentamente: 1.5 segundos
...
void loop()
{
	...
	if(1000<(millis()-tiempo_ant)) // Revisi�n peri�dica del estado de la bater�a (cada segundo)
	{ 
		tiempo_ant=millis();
		control_bateria();
		//Serial.print("Altura: ");
	...
	*/
	...
	if(interpolating == 0) //todo: convertir a get_ik_status() o check_interpolating_running()
	{
		//	tiempo_ant=millis();
		doIK();
...
void read_commands( void )
{
	...
	if (ps2x.NewButtonState()) // SOLO PARA PULSACI�N DE BOTONES
	{
	...
	{ 
		...
		if((ps2x.Read_Joystick(PSS_LX)) > 15 || ((ps2x.Read_Joystick(PSS_LX)) < -15) ){		//	EJE X
			...
			//Serial.print("rotaci�n sobre eje Y: ");
		...
		if((ps2x.Read_Joystick(PSS_LY)) > 15 || ((ps2x.Read_Joystick(PSS_LY)) < -15) ){		//	EJE Y
			...
			//Serial.print("rotaci�n sobre eje X: ");
		...
		//	Serial.print("rotaci�n sobre eje Z: ");
...
//	AVISOS SONOROS CUANDO ESTA EN ROJO?
	...
	{
		// este color era indicativo para las pruebas, EN LA VERSI�N FINAL DESACTIVA EL MOSFET_PIN PARA APAGAR EL ROBOT
		set_color(0,0,255);
		//si llegase a este if se
		// aunque por defecto est� a LOW
	...
	if (encendido==false){		   // SI NO HAY SUFICIENTE VOLTAJE EN LA BATER�A, EL PROGRAMA  NO SE INICIAR�
		digitalWrite(MOSFET_PIN, LOW);
		while(1);// se queda en bucle, hay que desconectar la bateria, CARGARLA,  y volverla a conectar
	}
	else digitalWrite(MOSFET_PIN, HIGH);// SI LA HAY SE ACTIVA EL MOSFET_PIN Y SE INICIA EL ROBOT
...
void setup_gpios( void )
{
	...
	pinMode(MOSFET_PIN, OUTPUT);// MOSFET_PIN
